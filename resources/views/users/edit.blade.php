            ...
            <div class="card ">
              ...
              <div class="card-body ">
                ...
                <div class="row">
                 ...
                 <div class="row">
                  <label class="col-sm-2 col-form-label">{{ __('Service Provider') }}</label>
                  <div class="col-sm-7">
                    <div class="form-group{{ $errors->has('serviceProviderId') ? ' has-danger' : '' }}">

                      <select class="form-control{{ $errors->has('serviceProviderId') ? ' is-invalid' : '' }}" name="serviceProviderId" id="input-serviceProvider"  placeholder="{{ __('Service Provider') }}">
                        <option>--Select--</option>
                        @foreach ($ServiceProviders as $item)
                      <option <?php $user['serviceProvider']['serviceProviderId']==$item['serviceProviderId']? print 'selected' :''?> value="{{$item['serviceProviderId']}}">{{$item['name']}}</option>
                        </option>
                        @endforeach
                      ...
                      @if ($errors->has('serviceProviderId'))
                        <span id="serviceProviderId-error" class="error text-danger" for="input-serviceProviderId">{{ $errors->first('serviceProviderId') }}</span>
                      @endif
                    </div>
                  </div>
                </div>
