          ...
          <form method="post" action="{{ route('user.store') }}" autocomplete="off" class="form-horizontal">
            ...
            <div class="card ">
              ...
              <div class="card-body ">
                ...
                <div class="row">
                  <label class="col-sm-2 col-form-label">{{ __('ServiceProvider') }}</label>
                  <div class="col-sm-7">
                    <div class="form-group{{ $errors->has('serviceProviderId') ? ' has-danger' : '' }}">
                      <select class="form-control{{ $errors->has('serviceProviderId') ? ' is-invalid' : '' }}" name="serviceProviderId" id="">
                        <option value="null">--Select ServiceProvider--</option>
                        @foreach ($errors as $serviceProvider)
                         <option value={{$serviceProvider->serviceProviderId}}>{{$serviceProvider['name']}}</option>
                        @endforeach
                      </select>
                      @if ($errors->has('serviceProviderId'))
                        <span id="name-error" class="error text-danger" for="input-name">{{ $errors->first('serviceProviderId') }}</span>
                      @endif
                    </div>
                  </div>
                </div>
