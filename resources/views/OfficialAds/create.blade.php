              ...
              <div class="card-header card-header-primary">
                ...
                <div class="row ">
                  <div class="col-md-12 text-right">
                      <a href="{{ route('OfficialAds.index') }}" class="btn btn-sm btn-primary">{{ __('Back to list') }}</a>
                  </div>
                </div>
                <div class="row">
                 <label class="col-sm-2 col-form-label">{{ __('Partner') }}</label>

                  <div class="col-sm-9">
                    <div class="form-group{{ $errors->has('partnerId') ? ' has-danger' : '' }}">
                    <select class="form-control{{ $errors->has('partnerId') ? ' is-invalid' : '' }}" name="partnerId" id="input-partnerId" placeholder="{{ __('Ads Title') }}" value="{{ old('partnerId') }}" required="true" aria-required="true">
                            <option value="">--Select--</option>
                        @foreach ($OfficialPartner as $item)
                                <option value="{{$item['partnerId']}}">{{$item['name']}}</option>
                         @endforeach
                    </select>                      @if ($errors->has('partnerId'))
                        <span id="title-error" class="error text-danger" for="input-partnerId">{{ $errors->first('partnerId') }}</span>
                      @endif
                    </div>
                  </div>
                </div>
